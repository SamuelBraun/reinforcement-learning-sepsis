...
\paragraph{Linear Q-learning collapses to a single action.} The Linear-Q training curve climbs from the random baseline (0.559) to a final snapshot return of 0.685 within 40{,}000 environment steps and stays perfectly flat thereafter. All three seeds collapse onto an identical asymptote, with the across-seed standard deviation vanishing after step 40{,}000. The action heatmap (\Cref{fig:configB-policies}) reveals the cause: 100\% of Linear-Q's on-policy state visits select action (V=none, F=none). The agent has converged to a constant policy.

This is not a tie-breaking artefact. The weights are initialised from $\mathcal{N}(0, 0.01^2)$ precisely to give different actions different initial $Q$-values, and three seeds with different initial $W$ still converge to the same single-action policy. The fixed point is robust to initialisation. The reason is representational: the linear functional form $Q(s,a) = W[a] \cdot x + b[a]$ cannot express the multi-modal optimal policy of this MDP, and under the bootstrapped target the linear fit settles on the constant policy that is closest to optimal in mean-return terms. Section \ref{sec:evaluation-A} provides the analytical justification: the always-action-0 policy reaches 92\% of $J^*$ on the clean MDP, so the TD updates have weak incentive to escape it.

\paragraph{DQN learns a non-degenerate policy but does not converge.} The DQN curve oscillates between 0.54 and 0.64 across the full 150{,}000-step budget without settling. The across-seed band stays wide. This matches DQN's documented behaviour with bootstrapped targets and replay on small networks; longer training, soft target updates, or a smaller learning rate could likely close the gap, but at the chosen budget DQN underperforms its potential. The deployed model is the per-seed best snapshot rather than the final one, captured by the evaluation callback.

\Cref{tab:configB-results} reports the Configuration B comparison. Mean returns rank Linear-Q (0.666) above DQN (0.608) above random (0.559), while survival ranks DQN (67.4\%) above Linear-Q (66.6\%) above random (65.0\%). The two metrics disagree.
...
Linear-Q & $0.666 \pm 0.472$ & $66.6$ $[63.7, 69.7]$ & $9.3$ & $40{,}000$ steps \\
DQN      & $0.608 \pm 0.469$ & $67.4$ $[64.3, 70.1]$ & $9.3$ & not reached \\
\bottomrule
\end{tabular}
...
\paragraph{Clinical interpretability.} DQN is the only Configuration B agent that responds to severity. Mean vasopressor level rises from approximately 1.0 at start-SOFA $< 4$ to 1.4 at start-SOFA $> 12$, a monotonic escalation of about 0.4 units on the 0--4 scale (\Cref{fig:configB-sofa}). Mean fluid level also rises mildly from 1.5 to 1.8 over the same range. Linear-Q is flat at 0 across the SOFA axis for both treatments, consistent with its single-action policy. DQN's escalation pattern is qualitatively the one Value Iteration showed in Configuration A: vasopressor moves with severity, fluid moves less. The magnitude is small, which matches DQN's small but real +2.4 percentage-point survival lift over the random floor.
...
DQN (Config B)                      & deep function approximation   & $0.608$ & $67.4$ \\
\bottomrule
\end{tabular}
\caption{Configuration A vs B comparison. The oracle row isolates the wrapper cost (0.3 pp) from the function-approximation cost ($\sim$11 pp).}
...
The decomposition argues that the clinical wrappers are not what the Configuration B agents are struggling against. The real cost is the observation regime: the best learning agent (DQN, 67.4\%) sits 11.1 percentage points below the oracle (78.5\%), and that entire gap comes from having to read a noisy partially observed 47-dimensional vector and generalise across states the agent never sees twice. The wrapper-to-observation-cost ratio is roughly 37:1 in favour of observation cost. Algorithm-class choice should follow the observation model, and the cost of moving from a tabular method to deep RL is measurable, non-zero, and worth quantifying for any deployment decision.
