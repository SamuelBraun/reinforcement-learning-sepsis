...
\paragraph{Linear Q-learning (raw and RFF variants).} Semi-gradient TD with two linear-in-features approximators tested as an ablation. The \emph{raw} variant uses $Q(s,a) = W[a] \cdot x + b[a]$ on the 47-dimensional observation directly. The \emph{Random Fourier Feature (RFF)} variant first projects $x$ through a fixed random non-linear basis $\phi_k(x) = \sqrt{2/D} \cos(\omega_k \cdot x + b_k)$ with $\omega_k \sim \mathcal{N}(0, I)$ and $b_k \sim \mathcal{U}(0, 2\pi)$, giving a $D = 256$-dimensional embedding, then applies the same semi-gradient TD update on $\phi$ as on $x$. RFF features approximate an RBF kernel as $D \to \infty$ and provide a much richer (but still linear-in-features) function class. The two variants share all other hyperparameters: $\alpha = 0.01$ for raw and $\alpha = 0.005$ for RFF (smaller because the dense 256-dim features amplify the gradient), 150{,}000 environment steps per seed across three seeds, $\epsilon$-decay $1.0 \to 0.05$. Weights are initialised from $\mathcal{N}(0, 0.01^2)$ to break argmax ties. The RFF variant tests whether the Linear-Q failure mode is representational (which RFF should escape) or structural (which it would not).
